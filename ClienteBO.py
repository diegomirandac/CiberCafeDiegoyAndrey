        ...
        try:
            if(self.validar(telefono)):
                if(not self.exist(telefono)): 
                    telefono.lastUser = ""
                    insertSQL = "INSERT INTO telefonos (`PK_telefono`, `PK_FK_cedula`, `descripcion`, `ultimavisita`) VALUES (%s, %s, %s, CURDATE())"
        ...
        try:
            if(self.validar(telefono)):
                ...
                if(self.exist(telefono)): 
                    telefono.lastUser = ""
                    updateSQL = "UPDATE telefonos  set `PK_FK_cedula` = %s, `descripcion` = %s, `ultimoususario` = CURDATE() WHERE `PK_telefono` =  %s"
