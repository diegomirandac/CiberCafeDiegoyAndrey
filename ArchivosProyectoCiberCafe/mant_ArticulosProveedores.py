...
#from modelo import ArticuloProveedores
from modelo import Cliente

#Incluye el objeto de logica de negocio
#from modelo import ArticuloProveedoressBO
from modelo import ClientoBO

class MantArticuloProveedoress():
    ...
    def __init__(self, parent):
        ...
        self.raiz.title ("Mantenimiento de ArticuloProveedores")
        ...
        #*************************************************************************
        self.ArticuloProveedores = ArticuloProveedores.ArticuloProveedores() #se crea el objeto de dominio para guardar la información
        ...
        self.lb_tituloPantalla = Label(self.raiz, text = "MANTENIMIENTO DE PROVEEDORES", font = self.fuente)
        ...
        self.txt_cedula = Entry(self.raiz, textvariable=self.ArticuloProveedores.cedula, justify="right", width=12)
        ...
        #coloca en el formulario el campo y el label de ArticuloProveedores
        self.lb_ArticuloProveedores = Label(self.raiz, text = "ArticuloProveedores:")
        self.lb_ArticuloProveedores.place(x = 240, y = 120)
        self.txt_ArticuloProveedores = Entry(self.raiz, textvariable=self.ArticuloProveedores.ArticuloProveedores, justify="right", width=30)
        self.txt_ArticuloProveedores.place(x = 370, y = 120)
        ...
        self.txt_descripcion = Entry(self.raiz, textvariable=self.ArticuloProveedores.descripcion, justify="right", width=30)
        ...
        self.sheet = Sheet(self.raiz,
                            ...
                            headers = ['Cédula', 'Nombre', 'ArticuloProveedores', 'Descripción'],
    ...
    def enviarInformacion(self):
        try:
            self.ArticuloProveedoresBo = ArticuloProveedoressBO.ArticuloProveedoressBO() #se crea un objeto de logica de negocio
            if(self.insertando == True):
                self.ArticuloProveedoresBo.guardar(self.ArticuloProveedores)
            else:
                self.ArticuloProveedoresBo.modificar(self.ArticuloProveedores)
            
            self.cargarTodaInformacion()
            self.ArticuloProveedores.limpiar()
    ...
    def limpiarInformacion(self):
        self.ArticuloProveedores.limpiar() #llama al metodo de la clase cliente para limpiar los atritudos de la clase
        self.nombreCliente.set("")
        self.insertando = True
    ...
    def consultarNombre(self):
        try:
            ...
            self.cliente.Cedula.set(self.ArticuloProveedores.Cedula.get()) #setea la cédula de la cliente
    ...
    def cargarTodaInformacion(self):
        try:
            self.ArticuloProveedoressBo = ArticuloProveedoressBO.ArticuloProveedoressBO() #se crea un objeto de logica de negocio
            resultado = self.ArticuloProveedoressBo.consultar()
    ...
    def cargarInformacion(self):
        try:
            ...
            ArticuloProveedores = (self.sheet.get_cell_data(datoSeleccionado[0],2))
            self.ArticuloProveedores.ArticuloProveedores.set(ArticuloProveedores)
            self.ArticuloProveedoresBo = ArticuloProveedoressBO.ArticuloProveedoressBO() #se crea un objeto de logica de negocio
            self.ArticuloProveedoresBo.consultarArticuloProveedores(self.ArticuloProveedores) #se envia a consultar
    ...
    def eliminarInformacion(self):
        try:
            ...
            resultado = msg.askquestion("Eliminar",  "¿Desear eliminar el ArticuloProveedores "+ArticuloProveedores+" de "+nombre+" de la base de datos?")
            if resultado == "yes":
                self.ArticuloProveedores.ArticuloProveedores.set(ArticuloProveedores)
                self.ArticuloProveedoresBo = ArticuloProveedoressBO.ArticuloProveedoressBO() #se crea un objeto de logica de negocio
                self.ArticuloProveedoresBo.eliminar(self.ArticuloProveedores) #se envia a consultar
                self.cargarTodaInformacion()
                self.ArticuloProveedores.limpiar()
                self.nombreCliente.set("")
